...
3.  Make a scatterplot of `bill_depth_mm` vs. `bill_length_mm`.
    ...
    ```{r}
    ggplot(data=penguins,mapping=aes(x=bill_length_mm, y=bill_depth_mm, colour=species))+
      geom_point() + 
      geom_smooth(method="lm")+
      labs(title="Bill Length vs Bill Depth", subtitle="For all 3 penguin species, bill length is positively associated with bill depth", x="Bill Length (mm)", y="Bill Depth (mm)", colour="Species")
    ```
...
4.  What happens if you make a scatterplot of `species` vs. `bill_depth_mm`?
    ...
    ggplot(penguins, aes(x=species, y=bill_depth_mm, colour=species)) +
      geom_boxplot() +
      labs(subtitle="A boxplot is a better choice to display the distribution of bill depth among differnt penguin species")
    ```
...
5.  Why does the following give an error and how would you fix it?
    ...
    penguins_filtered <- filter(penguins, sex=="male"|sex=="female")
    ggplot(data = penguins_filtered, aes(sex, body_mass_g)) + 
...
6.  What does the `na.rm` argument do in `geom_point()`?
    ...
    The na.rm arguement, meaning "n/a remove", prevents n/a values in the data from being included in the geom_point() scatterplot.
    na.rm = FALSE by default

    ```{r}
    ggplot(penguins, aes(x=body_mass_g, y=bill_depth_mm, colour=species)) + 
...
8.  Recreate the following visualization.
    ...
    #| bill_depth_mm should be mapped to colour to represent it as a continuous variable at the local geom_point level
...
9.  Run this code in your head and predict what the output will look like.
    ...
    #| PREDICTION: A scatterplot plotting flipper_length_mm vs body_mass_g with colours indicating which island the penguin data point is from. A smooth line will overlay to display the positive relationship of flipper length to body mass.
    #| RESULT: Prediction was relatively close, but did not consider that island is a globally mapped aesthetic. geom_smooth therefore shows 1 line for each island, with relatively positve linear relationships except for data from Torgersen island.
...
10. Will these two graphs look different?
    ...
    #| PREDICTION: The two graphs should look identical. The first code globally sets the data source and x and y variables, whie the second code does the same but locally instead for each geom
...
1.  Make a bar plot of `species` of `penguins`, where you assign `species` to the `y` aesthetic.
    ...
    #| Assigning species to the y axis creates a horizontal bar graph
    ```
...
2.  How are the following two plots different?
    ...
    #| the fill aesthetic is more useful as it changes the entire colour of the bars instead of just the outlining it like the colour aesthetic
...
3.  What does the `bins` argument in `geom_histogram()` do?
    ...
    #| The bins argument sets the amount of bars/bins in the histogram
    ```
...
4.  Make a histogram of the `carat` variable in the `diamonds` dataset that is available when you load the tidyverse package.
    ...
    #| binwidth = 0.08 to 0.1 displays differing level of detail in the distribution of carats, generally being left-skewed. 
    ```
